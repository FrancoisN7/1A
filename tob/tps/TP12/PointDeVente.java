...
public class PointDeVente {
	...
	public int getPrix(Carburant carburant, LocalDateTime date) {
		...
		if (prix.containsKey(carburant)) {
			NavigableMap<LocalDateTime, Integer> Dates_prix = prix.get(carburant);
			Map.Entry<LocalDateTime, Integer> Date_prix = Dates_prix.floorEntry(date);
			res = Date_prix == null ? 0 : Date_prix.getValue();
		}
		return res;
	}
