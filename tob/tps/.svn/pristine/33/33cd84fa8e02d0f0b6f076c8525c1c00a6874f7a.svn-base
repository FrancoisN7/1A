...
public class Cercle implements Mesurable2D {
	 ...
	 *  @param centrec est le centre du cercle
	 *  @param p est un point du cercle
	 */
    public static Cercle creerCercle(Point centrec, Point p) {
        assert centrec != null;
        assert p != null;
        assert centrec.getX() != p.getX() || centrec.getY() != p.getY();
        return new Cercle(centrec, centrec.distance(p));
    }
