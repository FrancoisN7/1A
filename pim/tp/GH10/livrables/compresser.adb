...
procedure Compresser is
    ...
    procedure construire_tab_noeuds(tab_noeuds : out tab_arbre.T_Tableau; Taille : out Integer; position_fichier : in integer) is
    ...
    begin
        taille := 0;
        open (file_txt, In_File, Argument(position_fichier)); 	-- Ouverture du fichier en lecture
    ...
    procedure construire_arbre(arbre : out T_Arbre; position_fichier : Integer) is
        function indice_min(tab : in tab_arbre.T_Tableau) return Integer is
            indice : Integer;
            min : integer;
        begin
            indice := 1;
            --chercher le premier element du tableau
            while est_vide(la_donnee(tab, indice)) loop
                indice := indice + 1;
            end loop;
            --trouver le min et son indice
        ...
        arbre_vide : T_Arbre;
    begin
        construire_tab_noeuds(tab_noeuds, taille, position_fichier);
    ...
    procedure afficher_tab_huffman(tab_huffman : in tab_lca.T_Tableau) is
        sda : T_LCA;
    begin
        for i in 1..tab_lca.longueur(tab_huffman) loop--afficher le code de chaque caractere
    ...
    begin
        ...
        --echanger le symbole de fin de fichier avec le debut du tableau
    ...
    procedure encoder_fichier(tab_infixe : in tab_entier.T_Tableau;
                              tab_huffman : in tab_lca.T_Tableau; structure_arbre : in out T_LCA; position_fichier : in Integer)is
        ...
        procedure encoder_texte(tab_huffman : in tab_lca.T_Tableau; compteur : in out Integer;
                                octet : in out T_octet; file_hff : in out Byte_file.file_type; position_fichier : in Integer) is
            file_txt : Ada.Text_IO.file_type;
            carac : Character;
            sda : T_LCA;
            taille_tab : integer;
        begin
            open (file_txt, In_File, Argument(position_fichier));
    ...
    begin
        byte_file.create(file_hff, byte_file.Out_File, Argument (position_fichier) & ".hff"); -- Création / écriture
        encoder_carac(tab_infixe, file_hff); --encoder le tab_infixe
        compteur := 0;
        octet := 0;
        encoder_sda(structure_arbre, compteur, octet, file_hff);
        encoder_texte(tab_huffman, compteur, octet, file_hff, position_fichier);
...
begin
    ...
    --construire l'arbre de huffman
    construire_arbre(arbre, position_fichier);

    --creer le tableau de huffman
    Initialiser(sda_binaire);
    creer_tab_huffman(arbre, tab_huffman, sda_binaire);
    vider(sda_binaire);

    --creer le tableau infixe
    parcours_infixe(arbre, tab_infixe);

    --creer le structure de l'arbre
    structurer_arbre(arbre, structure_arbre);

    --encoder le fichier
    encoder_fichier(tab_infixe, tab_huffman, structure_arbre, position_fichier);

    if Argument_Count = 2 then--afficher les etapes si c'est demandé
        put_line("arbre :");
