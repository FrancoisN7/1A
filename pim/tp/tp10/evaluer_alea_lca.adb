...
package LCA_I is
   ...
   begin
      for i in 1..Taille loop
         ...
         if Cle_Presente(Sda_ex,nombre_alea) then
            Enregistrer(Sda_ex, nombre_alea, La_Donnee(Sda_ex, nombre_alea)+1);
         else
            Enregistrer(Sda_ex, nombre_alea, 1);
         end if;
      end loop;
      Min:=La_Donnee(Sda_ex,1);
      Max:=La_Donnee(Sda_ex,1);
      for i in 2..Taille loop
         begin
            if Min>La_Donnee(Sda_ex,i) then
               Min:=La_Donnee(Sda_ex,i);

            elsif Max<La_Donnee(Sda_ex,i) then
               Max:=La_Donnee(Sda_ex,i);
