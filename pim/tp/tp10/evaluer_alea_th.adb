...
procedure Evaluer_Alea_TH is
   ...
   function Hachage_identite (x : Integer) return Integer is
   begin
      return x;
   ...
   begin
      for i in 1..Taille loop
         ...
         if Cle_Presente(T_ex,nombre_alea) then
            Enregistrer(T_ex, nombre_alea, La_Donnee(T_ex, nombre_alea)+1);
         else
            Enregistrer(T_ex, nombre_alea, 1);
         end if;
      end loop;
      Min:=La_Donnee(T_ex,1);
      Max:=La_Donnee(T_ex,1);
      for i in 2..Taille loop
         begin
            if Min>La_Donnee(T_ex,i) then
               Min:=La_Donnee(T_ex,i);

            elsif Max<La_Donnee(T_ex,i) then
               Max:=La_Donnee(T_ex,i);

            end if;
         exception
            when Cle_Absente_Exception => Min:=0;
         end;
